...
from app.avestan.exceptions import InvalidAlphabetDirection
from app.db.models.alphabet import WrittingDirection
from app.limits import LIMITS


class WordTransliterated(BaseModel):
    transliterated: str

    model_config = ConfigDict(from_attributes=True)


class TransliterateWord(BaseModel):
    ...
    def ensure_valid_direction(cls, v):
        if v not in [WrittingDirection.RTL, WrittingDirection.LTR]:
            raise InvalidAlphabetDirection(direction=v)
        return v
